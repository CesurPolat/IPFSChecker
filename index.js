...
gateways.forEach((element, i) => {
  ...
  axios
    .get(
      element + "bafybeifx7yeb55armcsxwwitkymga5xf53dxiarykms3ygqic223w5sk3m",
      { timeout: timeout }
    )
    .then(function (resp) {
      if (resp.data == "Hello from IPFS Gateway Checker\n") {
        table.push([element, (Date.now() - startTime) / 1000]);
        clear(true);
        console.log(table.toString());
      }
    })
    .catch(() => {
      /* table.push([element,"X"])
    clear(true);
    console.log(table.toString()); */
    });
});
